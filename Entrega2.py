...
def leer_datos_json(ruta_archivo):
    """
    Lee datos desde un archivo JSON de forma segura.
...
def ingresar_turista():
    """
    Registra un nuevo turista y lo guarda en el archivo JSON.
    """
    ...
    while True:
        ...
        if dni and validar_dni(dni):
            break
        print("Error: El DNI solo debe contener números y puntos.")
...
def listar_turistas_activos():
    ...
    for datos in turistas.values():
        if datos.get("activo", False):
            ...
            telefonos_str = ", ".join(filter(None, telefonos.values()))
...
def cargar_paquete():
    """
    Registra un nuevo paquete y lo guarda en el archivo JSON.
    """
...
def modificar_paquete():
    ...
    if id_modificar in paquetes and paquetes[id_modificar].get("activo", False):
        ...
        while True:
            print(f"\n¿Qué desea modificar?")
            print("[1] Datos generales del paquete")
            print("[2] Servicios del paquete")
...
def alta_contrato():
    ...
    forma_pago = input("Ingrese medio de pago a utilizar (Efectivo, Transferencia, Tarjeta): ").strip()
    ...
    contratos[id_contrato] = {
        ...
        "activo": True,
        "fecha": fecha_contrato.strftime("%d/%m/%Y %H:%M:%S"),
        "idTurista": id_turista,
...
def baja_contrato():
    ...
    if id_contrato in contratos and contratos[id_contrato].get("activo", False):
        ...
        if confirmacion == 'S':
            ...
            contratos[id_contrato]["fechaBaja"] = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            escribir_datos_json(ARCHIVO_CONTRATOS, contratos)
    ...
    if id_contrato in contratos and contratos[id_contrato].get("activo", False):
        ...
        while True:
            ...
            if opcion == '1':
                ...
                print(f"Total: ${contrato_a_modificar['Total']:,.2f}")
            elif opcion == '2':
...
def listar_operaciones_del_mes():
    """
    ...
    for id_contrato, datos_contrato in contratos.items():
        if datos_contrato.get("activo", False):
            try:
                fecha_contrato = datetime.datetime.strptime(datos_contrato["fecha"], "%d/%m/%Y %H:%M:%S")

                if fecha_contrato.month == hoy.month and fecha_contrato.year == hoy.year:
                    se_encontraron_operaciones = True
                    
                    id_turista = datos_contrato['idTurista']
                    id_paquete = datos_contrato['idPaquete']                    
...
def reporte_resumen_montos_por_mes():
    """
...
def informe_paquetes_menos_vendidos():
    """
    ...
    for id_p, datos_p in ventas_por_paquete.items():
        lista_paquetes_ventas.append({
            ...
            "ventas": datos_p["ventas"]
        })
    
    lista_ordenada = []
    copia_lista = list(lista_paquetes_ventas)
    
    while copia_lista:
        min_paquete = copia_lista[0]
        for paquete in copia_lista:
            if paquete["ventas"] < min_paquete["ventas"]:
                min_paquete = paquete
        lista_ordenada.append(min_paquete)
        copia_lista.remove(min_paquete)

    print(f"\n{'Posición':<10}{'Nombre del Paquete':<40}{'Cantidad de Ventas':<20}")
    print("-" * 75)
    
    for i, paquete in enumerate(lista_ordenada[:5]):
        posicion = f"{i + 1}."
    ...
    while True:
        ...
        if opcion == "1":
            while True:
                ...
                if sub_opcion == "1": ingresar_turista()
                elif sub_opcion == "2": modificar_turista()
                elif sub_opcion == "3": eliminar_turista()
                elif sub_opcion == "4": listar_turistas_activos()
            ...
            while True:
                ...
                if sub_opcion == "1": cargar_paquete()
                elif sub_opcion == "2": modificar_paquete()
                elif sub_opcion == "3": eliminar_paquete()
                elif sub_opcion == "4": listar_paquetes_activos()
                elif sub_opcion == "0": break
                else: print("Opción no válida.")
                input("\nPresione ENTER para continuar")
             ...
             while True:
                ...
                if sub_opcion == "1": alta_contrato()
                elif sub_opcion == "2": baja_contrato()
                elif sub_opcion == "3": modificar_contrato()
                elif sub_opcion == "0": break
                else: print("Opción no válida.")
                input("\nPresione ENTER para continuar")
            ...
            while True:
                ...
                if sub_opcion == "1":   listar_operaciones_del_mes()
                elif sub_opcion == "2": resumen_cantidad_contratos_por_mes()
                elif sub_opcion == "3": reporte_resumen_montos_por_mes()
                elif sub_opcion == "4": informe_paquetes_menos_vendidos()
                elif sub_opcion == "0": break
