...
def verificaIDpaquete(_idPaquete, _paquetes): ################# HACER DOCSTRING
    ...
    formatoNombrePaquete = r"^PQT\d{3}$"
    if re.match(formatoNombrePaquete, _idPaquete) and _idPaquete in _paquetes:
...
def verificaIDcontrato(_idContrato, _contratos):
    ...
    if _idContrato in _contratos:
        ...
        print("Contrato no encontrado. Intente de nuevo.")
...
def bajaContrato(_paquetes, _contratos):
    ...
    _verificaNumeroDePaqueteBool, _verificaNumeroDeContratoValor= verificaIDcontrato(_idContrato, _contratos)
    
    if _verificaNumeroDePaqueteBool == True: 
        cancelado= True 
        _fechaDeBaja= datetime.datetime.now()
        print("Cancelado con éxito. Fecha de cancelación: ", _fechaDeBaja)
        
        contratoAeliminar= _contratos.pop(_verificaNumeroDeContratoValor) 
...
def modficarContrato(_paquetes):
    """
...
def main():
    ...
    "20000067890123": {
        ...
        "Total": paquetes["PQT010"]["valor"] * 10,
        "Medios de pago": {
