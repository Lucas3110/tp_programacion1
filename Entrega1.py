...
def cargar_datos_json():
    ...
    Si el archivo no existe o está dañado, lo crea vacío.
    Devuelve tres diccionarios: turistas, paquetes y contratos.
    """
    try:
...
def guardar_datos_json(turistas, paquetes, contratos):
    """
    Guarda los diccionarios de turistas, paquetes y contratos en el archivo data.json.
...
def generar_id(turistas):
    """
    Genera un nuevo ID para un turista, sumando 1 al ID más alto existente.
    Devuelve el nuevo ID como string.
    """
    if len(turistas) == 0:
        return "1"
    else:
...
def cargarServicios():
    """
    Solicita al usuario los detalles de los servicios de un paquete (vuelo, alojamiento, etc).
    Devuelve un diccionario con los servicios ingresados.
    """
    servicios = {}
...
def cargarServiciosParaModificar(servicios_anteriores):
    """
    Permite modificar los servicios de un paquete.
    Muestra los servicios actuales y permite cambiarlos o mantenerlos.
    Devuelve el nuevo diccionario de servicios.
    """
    servicios = {}
...
def mostrarPaquete(id_paquete, datos):
    ...
    Recibe el ID y el diccionario de datos del paquete.
    """
    try:
        print("ID:", id_paquete)
...
def mostrarServicios(servicios):
    ...
    Recibe un diccionario con los servicios y sus detalles.
    """
    print("\nServicios:")
...
def verificaIDcontrato(_idContrato, _contratos): 
    """
    Verifica si el ID de contrato existe y está activo.
    Devuelve una tupla (True/False, ID).
    """ 
...
def listar_operaciones_mes():
    ...
    Incluye fecha, cliente, producto, cantidad, valor unitario y total.
    """
    try:
...
def resumen_cantidad_contratos_por_mes():
    """
    Muestra un resumen anual de la cantidad de contratos por paquete y por mes.
    Lee los datos del archivo y los muestra en formato de tabla.
    """
    try:
        turistas, paquetes, contratos = cargar_datos_json()
...
def main():
    ...
    Muestra el menú principal y permite navegar por las distintas opciones del sistema.
    """
    while True:
        print()
        print("---------------------------")
